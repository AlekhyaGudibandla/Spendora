...
import type { Role } from '@/store/dashboard-store'
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <motion.header
        ...
        className="relative z-20 flex flex-col gap-6 border-b border-edge/60 pb-8 pt-2 md:flex-row md:items-end md:justify-between"
      >
        <div>
          ...
          <p className="mt-2 max-w-md text-sm leading-relaxed text-muted">
            Layered cashflow intelligence — calm, architectural, built for
            focus.
          </p>
        </div>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <Switch
                id="role-mode"
                checked={isAdmin}
                onCheckedChange={(c) => onRoleChange(c ? 'admin' : 'viewer')}
              />
              <Label htmlFor="role-mode" className="text-[11px] normal-case">
                Admin
              </Label>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                variant="outline"
                size="icon"
                className="rounded-xl border-edge bg-panel/50"
